...
public class OrderOutboxPublisher {
    ...
    @Scheduled(fixedDelayString = "${order.outbox.publish-interval-ms:1000}")
    @Transactional
    public void publishPending() {
        ...
        for (OutboxMessageEntity row : batch) {
            try {
                ...
                log.info("Published outbox {} rk={} aggregate={}", row.getId(), row.getRoutingKey(), row.getAggregateId());
            } catch (Exception e) {
                log.error("Failed to publish outbox {}", row.getId(), e);
            }
        }
    }
}
